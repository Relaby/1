...
def create_app(config_class=Config):
    ...
    def before_request():
        ...
        if "Roblox" not in BrowserUserAgent:
            ...
            requestReferer = request.headers.get("Referer", default=None)
            if requestReferer:
                try:
                    if urlparse(requestReferer).hostname[-len(config_class.BaseDomain):] != config_class.BaseDomain:
                        logging.warning(f"Bad Referer - ref : {requestReferer} - target : {request.url}")
                        return "Bad Referer", 403
                except:
                    pass 
    
  # Make sure to return the app at the end
